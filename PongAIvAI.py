...
class FloatingRect:
    """
    pygame's Rect class can only be used to represent whole integer
    vertices, so we create a rectangle class that can have floating point
    coordinates
    """
    ...
    def move(self, x, y):
        return FloatingRect((self.pos[0] + x, self.pos[1] + y), self.size)
...
class Paddle:
    def __init__(self, pos, size, speed, max_angle, facing, timeout):
        self.frect = FloatingRect((pos[0] - size[0] / 2, pos[1] - size[1] / 2),
                                  size)
        self.speed = speed
        self.size = size
        self.facing = facing
    ...
    def move(self, enemy_frect, ball_frect, table_size):
        direction = self.move_getter(self.frect.copy(), enemy_frect.copy(),
                                     ball_frect.copy(), tuple(table_size))

        # direction = timeout(self.move_getter, (self.frect.copy(),
        # enemy_frect.copy(), ball_frect.copy(), tuple(table_size)), {},
        # self.timeout)
        if direction == "up":
    ...
    def get_face_pts(self):
        return ((self.frect.pos[0] + self.frect.size[0] * self.facing,
                 self.frect.pos[1]),
...
class Ball:
    def __init__(self, table_size, size, paddle_bounce, wall_bounce,
                 dust_error, init_speed_mag):
        rand_ang = (.2 + .6 * random.random()) * math.pi * (
            1 - 2 * (random.random(
    ...
    def get_center(self):
        return (self.frect.pos[0] + .5 * self.frect.size[0],
    ...
    def move(self, paddles, table_size):
        ...
        for wall_rect in walls_rects:
            if self.frect.get_rect().colliderect(wall_rect):
                ...
                while self.frect.get_rect().colliderect(wall_rect):
                    self.frect.move_ip(-.1 * self.speed[0],
                                       -.1 * self.speed[1])
                    c += 1  # this basically tells us how far the ball has
                    # traveled into the wall
                r1 = 1 + 2 * (random.random() - .5) * self.dust_error
                r2 = 1 + 2 * (random.random() - .5) * self.dust_error

                self.speed = (self.wall_bounce * self.speed[0] * r1,
                              -self.wall_bounce * self.speed[1] * r2)
                while c > 0 or self.frect.get_rect().colliderect(wall_rect):
                    self.frect.move_ip(.1 * self.speed[0], .1 * self.speed[1])
                    c -= 1  # move by roughly the same amount as the ball had
                    #  traveled into the wall
                moved = 1
                # print "out of wall, position, speed: ", self.frect.pos,
                # self.speed

        for paddle in paddles:
            if self.frect.intersect(paddle.frect):
                if (paddle.facing == 1 and self.get_center()[0] <
                        paddle.frect.pos[0] + paddle.frect.size[0] / 2) or \
                        (paddle.facing == 0 and self.get_center()[0] >
                         paddle.frect.pos[0] + paddle.frect.size[0] / 2):
                    continue

                c = 0

                while self.frect.intersect(paddle.frect) \
                        and not self.frect.get_rect().\
                                colliderect(walls_rects[0]) and \
                        not self.frect.get_rect().colliderect(walls_rects[1]):
                ...
                # Bona fide hack: enforce a lower bound on horizontal speed
                # and disallow back reflection

                # ball is not traveling (a) away from paddle (b) at a
                # sufficient speed
                if v[0] * (2 * paddle.facing - 1) < 1:

                    # transform y velocity so as to maintain the speed
                    # note that minimal horiz speed will be lower than we're
                    # used to, where it was 0.95 prior to increase by * 1.2
                ...
                while c > 0 or self.frect.intersect(paddle.frect):
                    ...
                    c -= 1
                # print "pos final: (" + str(self.frect.pos[0]) + "," + str(
                # self.frect.pos[1]) + ")"
        ...
        # if we didn't take care of not driving the ball into a wall by
        # backtracing above it could have happened that
        # we would end up inside the wall here due to the way we do paddle
        # bounces
        # this happens because we backtrace (c++) using incoming velocity,
        # but correct post-factum (c--) using new velocity
        # the velocity would then be transformed by a wall hit, and the ball
        #  would end up on the dark side of the wall
...
def directions_from_input(paddle_rect, other_paddle_rect, ball_rect,
                          table_size):
...
def timeout(func, args=(), kwargs={}, timeout_duration=1, default=None):
    """From:
    http://code.activestate.com/recipes/473878-timeout-function-using
    -threading/"""
    import threading
...
def game_loop(screen, paddles, ball, table_size, clock_rate, turn_wait_rate,
              score_to_win, display):
    score = [0, 0]
    ...
    while max(score) < score_to_win:
        ...
        if score != old_score:
            ...
            if score[0] != old_score[0]:
                screen.blit(font.render("Left scores!", True, white, black),
                            [0, 32])
            else:
...
def init_game():
    ...
    paddles = [
        Paddle((20, table_size[1] / 2), paddle_size, paddle_speed, max_angle,
               1, timeout),
        Paddle((table_size[0] - 20, table_size[1] / 2), paddle_size,
               paddle_speed, max_angle, 0, timeout)]
    ball = Ball(table_size, ball_size, paddle_bounce, wall_bounce, dust_error,
                init_speed_mag)

    # pick your combatants
    import chaser_ai
    import skypong
    ...
    game_loop(screen, paddles, ball, table_size, clock_rate, turn_wait_rate,
              score_to_win, 1)
    screen.blit(
        pygame.font.Font(None, 32).render(str('SWITCHING SIDES'), True, white),
        [int(0.6 * table_size[0]) - 8, 0])

    pygame.display.flip()
    clock.tick(4)

    paddles[0].move_getter, paddles[1].move_getter = paddles[1].move_getter, \
